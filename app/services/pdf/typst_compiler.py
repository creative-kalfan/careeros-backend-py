...
def render_document_model_to_typst(doc_model: ResumeDocumentModel) -> str:
    ...
    out: list[str] = [
        ...
        # NOTE: par(spacing:) needs Typst 0.12+; the pinned 0.11.1 uses
        # block spacing for inter-paragraph gaps.
    ...
    if hdr.full_name:
        out.append(f'  #text(size: 20pt, weight: "bold", fill: rgb("#0f172a"))[#t({_lit(hdr.full_name)})]')
    if hdr.headline:
        out.append(f'  #v(2pt)\n  #text(size: 11pt, weight: "bold", fill: rgb("#475569"))[#t({_lit(hdr.headline)})]')
    contact = hdr.contact_line()
    if contact:
        out.append(f'  #v(2pt)\n  #text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(contact)})]')
    out.append("]")
    out.append("")
    ...
    def _experience_block(positions: Any, heading: str) -> None:
        ...
        for exp in positions:
            ...
            out.append(f'  [#text(size: 10pt, weight: "bold", fill: rgb("#0f172a"))[#t({_lit(title)})]],')
            out.append(f'  [#text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(date_loc)})]],')
            out.append(")")
            ...
            for sub in getattr(exp, "sub_engagements", None) or []:
                ...
                if sub.description:
                    out.append(f'  #pad(left: 16pt)[#text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(sub.description)})]]')
    ...
    for sec in doc_model.section_order:
        if sec == "summary" and doc_model.summary and (doc_model.summary.text or "").strip():
            ...
            for prj in doc_model.projects:
                ...
                out.append(f'  #text(size: 10pt, weight: "bold", fill: rgb("#0f172a"))[#t({_lit((prj.name or "Project") + tech)})]')
                if prj.url:
                    out.append(f'  #text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(prj.url)})]')
                bits: list[str] = []
                if prj.description:
            ...
            for edu in doc_model.education:
                ...
                out.append("#grid(columns: (1fr, auto), gutter: 8pt,")
                out.append(f'  [#text(size: 10pt, weight: "bold", fill: rgb("#0f172a"))[#t({_lit(left)})]],')
                out.append(f'  [#text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(edu.date_range)})]],')
                out.append(")")
                meta = " | ".join(p for p in (edu.location, f"GPA: {edu.gpa}" if edu.gpa else "") if p)
                if meta:
                    out.append(f'  #text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit(meta)})]')
                if edu.coursework:
                    out.append(f'  #text(size: 8.5pt, fill: rgb("#64748b"))[#t({_lit("Coursework: " + ", ".join(edu.coursework))})]')
                out.append("#v(3pt)")
            ...
            if rows:
                ...
                for cat, skills in rows:
                    out.append(f'  #text(size: 9.5pt)[#strong[#t({_lit(cat + ":")})] #t({_lit(", ".join(skills))})]')
